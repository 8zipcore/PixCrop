...
class PixCropFrameManager {
  func dragFrame(
    ...
    let minSize = CGSize(
      width: PixCropFrame.minWidth,
      height: PixCropFrame.minHeight
    )
    
    newFrameSize.width -= widthDifferenceValue
    newFrameSize.height -= heightDifferenceValue
    
    let resizeFrame = resizeFrameToFitImage(
      locationType: selectionBox.locationType,
      newRect: CGRect(
        origin: newFrameCenter,
        size: newFrameSize
      ),
  ...
  func dragFrameEnded(
    ...
    let imageScale = LayoutUtils.scale(
      size: PixCropFrame.size,
      viewSize: viewSizeWithoutPadding
    )
  ...
  private func resizeFrameToFitImage(
