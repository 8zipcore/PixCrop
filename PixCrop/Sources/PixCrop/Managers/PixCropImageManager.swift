...
extension PixCropImageManager{
  ...
  func rotate(
    viewSize: CGSize,
    viewSizeWithoutPadding: CGSize,
    direction: PixCropRotateDirection
  ) {
    let viewCenter = LayoutUtils.center(for: viewSize)
    ...
    let newFrameSize = LayoutUtils.scaledSizeToFit(
      size: CGSize(
        width: PixCropFrame.size.height,
        height: PixCropFrame.size.width
      ),
    ...
    let swapImageSize = LayoutUtils.scaledSizeToFit(
      size: CGSize(
        width: PixCropImage.size.height,
        height: PixCropImage.size.width
      ),
      viewSize: viewSizeWithoutPadding
    )
    ...
    let scaleX = (newImageHeight * newZoomScale) / (lastImageSize.width * lastZoomScale)
    let newFrameCenterX = imageCenter.x + (viewCenter.x - imageCenter.x) * scaleX
    let scaleY = (newImageWidth * newZoomScale) / (lastImageSize.height * lastZoomScale)
    let newFrameCenterY = imageCenter.y + (viewCenter.y - imageCenter.y) * scaleY
    // 2. 1번 좌표를 imageCenter 기준으로 n도 회전환 좌표를 구한다.
    let rotatePosition = rotateUsingCGAffineTransform(
      point: CGPoint(x: newFrameCenterX, y: newFrameCenterY),
      center: imageCenter,
      angle: rotateDegree
    )
