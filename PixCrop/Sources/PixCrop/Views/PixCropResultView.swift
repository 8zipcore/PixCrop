...
public class PixCropResultView: UIView {
  ...
  private func configureView() {
    ...
    self.imageView.transform = CGAffineTransform(
      scaleX: PixCropImage.transformScale.x * newZoomScale,
      y: PixCropImage.transformScale.y * newZoomScale
    ).rotated(by: PixCropImage.degreeToRadian)
    /* Image 좌표 변환 */
    // 1. viewSize 변경 전 Frame과 Image의 TopLeading 좌표를 구함
    let frameTopLeading = LayoutUtils.topLeadingPosition(
      size: PixCropFrame.size,
    ...
    let newFrameTopLeading = LayoutUtils.topLeadingPosition(
      size: newFrameSize,
